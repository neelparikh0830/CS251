...
class graph {
  ...
  public:
        ...
        int dest, vector<int> & path) 
    {
      ...
      if (dest <= rpt.size())
      {
        vector<vertex_label> love;
        vector<int> nike;
        int i;
        love = rpt;
        for (i = dest; i != love[i].pred; i = love[i].pred)
        {
          if(love[i].pred == -1)
          {
            path.clear();
    ...
    bool dag_critical_paths(vector<vertex_label> & rpt) 
    {
      ...
      {
        for (edge &e : vertices[Adidas[i]].outgoing)
        {
          if (!(rpt[e.vertex_id].dist >= rpt[Adidas[i]].dist + e.weight))
          {
            rpt[e.vertex_id].dist = rpt[Adidas[i]].dist + e.weight;
            rpt[e.vertex_id].pred = Adidas[i];
          }
        }
      }
      return true;
    ...
    bool valid_topo_order(const vector<int> & order) {
      if(has_cycle())
        return false;
      vector<bool> arrayVisited = vector<bool>(num_nodes());

      for (int i = 0; i < arrayVisited.size(); arrayVisited[i] = false, i++);
      ...
      for (int i = 0; i < order.size(); i++)
      {
        if (!(vertices[order[i]].incoming.size() <= 0))
        {
          ...
          {
            if (!(arrayVisited[vertices[order[i]].incoming[j].vertex_id] != false))
            {
              return false;
            }
          }
        }
        arrayVisited[order[i]] = true;
      }
      return true;
    }
    ...
    bool enum_paths(int target, vector<string> &paths)
    {
      ...
      for(vertex &z : vertices)
      {
        ...
        {
          route.push_back(z.id);
          r.push(route);
          while(!r.empty())
          {
            route = r.front();
            r.pop();
            if (route[route.size()-1] != target)
            {
              ;
            }
            else
            {
              paths.push_back(id_vec2string(route));
            }
            for(edge &e : vertices[route[route.size()-1]].outgoing)
            {
              vector<int> dude(route);
              dude.push_back(e.vertex_id);
              r.push(dude);
            }
          }
        }
