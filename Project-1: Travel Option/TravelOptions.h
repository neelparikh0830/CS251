...
class TravelOptions
{
   ...
   */
    ...
    */
	...
	{
		for (Node* p = front; p != nullptr && p->next != nullptr; p = p->next)
		{
    ...
    */
	...
	{
		for (Node* p = front; p->next != nullptr; p = p->next)
		{
			for (Node* q = p->next; q != nullptr; q = q->next)
			{
				if (compare(p, q) != incomparable)
				{
					return false;
				}
			}
  ...
  bool is_pareto_sorted() const
    {
      ...
      // {
      //   if (((p->price > q->price) || (p->time < q->time)) || compare(p, q) == equal)
      //   {
      //       return false;
      //   }
      //   else
      ...
      // {
      //   if (((p->price > p->next->price) && (p->time < p->next->time)) || compare(p, p->next) == equal)
      //     return false;
    ...
    bool insert_sorted(double price, double time) 
	{
		...
		Node* wen = new Node(price, time, nullptr);
		Node* temp = front;
		Node* prev = nullptr;
		while(temp != nullptr && (price > temp->price || (price == temp->price && time > temp->time)))
		{
			prev = temp;
			temp = temp->next;
		}
    ...
    bool insert_pareto_sorted(double price, double time)
	{
		...
		Node* love = new Node(price, time, nullptr);
	  Node* p = front;
    Node* q = nullptr;
    Node* s;
    for(; p != nullptr && price > p->price; p = p->next)
    {
      if (time < p->time)
      {
        q = p;
    ...
    if (p != nullptr && p->price == price && time >= p->time)
    {
      return true;
    }

    for(; p != nullptr && time <= p->time; p = s)
    {
      s = p->next;
      delete p;
    }
   ...
   */
   TravelOptions * join_plus_plus(const TravelOptions &other) const 
    {
      TravelOptions* Neel = new TravelOptions();
      Node* q = other.front;
      for (Node* p = front; p != nullptr; p = p->next)
      {
        while (q != nullptr)
        {
          Neel->insert_pareto_sorted(q->price + p->price, q->time + p->time);
          q = q->next;
        }
        q = other.front;
      }
   ...
   TravelOptions * split_sorted_pareto(double max_price) 
    {
      ...
      TravelOptions* love = new TravelOptions();
      Node* p = nullptr;
      Node* q = front;
      for (; q != nullptr && q->price <= max_price; p = q, q = q->next);
      if (p != nullptr)
      {
        love->front = q;
        p->next = nullptr;
      ...
      return love;  // placeholder to make compiler happy with skeleton
    }
   ...
   */
  unsigned long long int checksum() const {
    unsigned long long int s = 0;
    Node *p = front;

    while (p != nullptr)
    {
      s = s ^ ((unsigned long long int)p);
      p = p->next;
    }
    return s;
  }
